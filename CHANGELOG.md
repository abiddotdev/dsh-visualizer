...
- Inline artifact preview in the gallery: clicking a row expands a live render of its already-served page directly beneath it (an accordion — one row at a time, click again to collapse) — a sandboxed `<iframe>` for HTML exports, a plain `<img>` for SVG (browsers never execute script for images regardless of content). The HTML branch fetches the page as text and embeds it via `srcDoc` rather than navigating an iframe's `src` to it, so it's never subject to `X-Frame-Options` regardless of the exports route's origin. The row's trigger is a native button spanning its badge/title/meta (not just the title text) with a rotating disclosure triangle, so the preview is reachable and operable by keyboard alone. No new server-side rendering: the preview loads the identical URL Open/Copy-link already address, inheriting the same sandboxing, CSP, and no-store headers.
