...
describe('visualizer-stream node', () => {
  it('publishes a streaming card once the html prefix is decodable', () => {
    ...
    expect(data(value)?.cards).toEqual([{ callId: 'call-1', phase: 'streaming', title: 'Dash', height: 320, html: '<p>rev', loadingMessages: [] }])
  })
  ...
  it('commits the complete document at block-end and keeps the same node key', () => {
    ...
    expect(data(value)?.cards).toEqual([{ callId: 'call-1', phase: 'complete', title: 'Dash', height: 320, html: CALL_ARGS.html, loadingMessages: [] }])
  ...
  it('marks an unterminated stream interrupted when the step closes without a final message', () => {
    ...
    expect(data(value)?.cards).toEqual([{ callId: 'call-1', phase: 'interrupted', title: 'Dash', height: 320, html: '<p>rev', loadingMessages: [] }])
  })
  ...
  it('withdraws the evidence on model retry and re-anchors a fresh attempt', () => {
    ...
    expect(data(second)?.cards).toEqual([{ callId: 'call-2', phase: 'streaming', title: null, height: null, html: '<p>two', loadingMessages: [] }])
  })
  ...
  it('produces the same settled result through replay as live append produced', () => {
    ...
    expect(replayed?.cards).toEqual([{ callId: 'call-1', phase: 'complete', title: 'Dash', height: 320, html: CALL_ARGS.html, loadingMessages: [] }])
  })
})
