...
The default branch tracks the **latest DeepSeek Harness pre-release** (`0.1.2-rc.1` or newer). `0.1.2-alpha.3` renamed the client `conversationEvents` service to `uiConversation` and moved the Chat Node out of `dsh-client-ui-conversation` into the new `dsh-client-ui-chat` package, so the card cannot mount on builds older than that — the tool parks at `pending (waiting for service: conversationEvents)`. `0.1.2-rc.1` replaced alpha.3's blanket "N tool calls" Compact-transcript fold with a turn-process fold that also swallows a settled card once its turn closes; this plugin now republishes every settled document through the harness's `conversation.chat.turnTail` chain (the same exemption `ui-deliverables`' produced-files row relies on) exactly once the turn closes, and only in Compact view, so cards stay visible in both view modes without a Settings workaround and without ever showing twice — see [Settled cards and Compact transcript view](#settled-cards-and-compact-transcript-view) below.
...
Harness `0.1.2-rc.1` folds every chat-node kind that isn't on a fixed independent-kind allowlist behind one disclosure once its turn closes — including the tool-call row a settled visualizer card renders into. `turn-tail` is on that allowlist, but it's also a hard mount boundary: the harness does not create that chat node until `turn/end`, so nothing can stream or settle into it early. The card therefore keeps rendering in place — full frame and chrome — for as long as the turn stays open, exactly as it always has; only once the turn actually closes does a second copy also publish through `conversation.chat.turnTail`, so the document survives the fold in Compact view. That handoff is a fresh mount (a new frame, scripts restarting), so it's the one visible transition, and it happens exactly once, only when the turn ends — never at the moment the call itself settles. A running or failed call never reaches turn-tail — both settle in place before any turn can close.

Normal transcript view never folds anything, so the in-place row's own copy already survives on its own — a second turn-tail copy there would be pure duplication. This plugin reads harness `ui-chat`'s own Compact/Normal preference directly from its durable settings section and stays empty at turn-tail while Normal view is active. That preference is stored under a namespace and field `ui-chat`'s public client API does not export (`packages/client/ui-chat/src/chat-settings.ts` upstream) — reading it means hardcoding those same private strings, a coupling to an unexported implementation detail with no compatibility guarantee. If a future harness release renames them, the read silently degrades to `undefined` and this plugin falls back to treating the turn as Compact — i.e. it keeps showing the turn-tail copy, so the worst-case regression is the harmless duplication above, never a document disappearing.
