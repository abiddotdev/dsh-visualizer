...
- id: dsh-visualizer
  config:
    ...
    exportDir: ~/.dsh/visualizer/exports   # default ($DSH_HOME honored); where exports are mirrored and served from
    maxExportAgeDays: 7    # default; exports older than this sweep at activation, 0 disables expiry
```
...
On the web profile the host half runs a second, read-only projection of every `visualizer` stream. The same `assistant/chunk` `tool-call-delta` events the browser card folds also reach the host's `session/event` firehose; the export fanout folds them identically and mirrors the growing document into the exports directory (`exportDir`, default `$DSH_HOME/visualizer/exports`, created `0700`) while the model is still writing — a `<base>.partial` sidecar grows token by token, then the `tool/call` event that precedes execution carries the authoritative bytes, which land under their final name by an atomic rename: `<base>.html`, or `.svg` for a bare SVG document. A failed render cleans up after itself — an error result, an interrupted step, or a provider retry removes what it streamed — so the directory holds settled documents only. The write path is contained: a fanout failure logs one line and never reaches the conversation, and nothing touches the workspace; exports live under the harness home.

The share icon on a settled card opens that page at a suburl of the harness web UI — `/visualizer/<name>?k=<token>`, the route matching the plugin's on-disk subtree — served by a prefix route the plugin registers on the harness web server (the plugin is the `/visualizer` namespace; no other feature claims it), with the same network-egress CSP the sandboxed shell enforces plus hardening headers on every answer: `nosniff`, `cache-control: no-store` (a shareable page must never be served stale from a cache), `referrer-policy: no-referrer`, `x-frame-options: DENY`, `cross-origin-resource-policy`, and a locked-down `permissions-policy`. Only a regular file of the exports directory is servable — a symlink planted there refuses with the same not-found page as any unknown name — and every request must echo the per-boot capability token (`?k=`) the host announces to its pages through the boot table; anything else takes that same not-found answer, so links stop being enumerable and holding the token is the access grant. That means links expire when the harness restarts (a re-render regenerates them) and anyone you send a link to can open it until then — this is unguessability, not per-user auth. An HTML export never executes on the harness origin itself: the route answers with a minimal wrapper page framing the document in `<iframe sandbox="allow-scripts" srcdoc="…">`, so generated scripts run in an opaque origin and inherit the wrapper's policy into the frame. A bare SVG serves raw (it stays hotlinkable as an image) under a policy variant whose `script-src` is `'none'` — diagrams carry no script, and `<img>`-embedded copies would not run it either. The name is derived in one shared module both planes import, so the URL always matches a file the host wrote. Names carry a kebab-case ASCII slug of the title — lowercase, hyphen-separated, unicode stripped (the card's display text keeps the full title; the artifact never does) — plus a digest for uniqueness: `<slug>-<16 hex digits of an FNV-1a digest over the original title and bytes>.html|.svg`. One exact render maps to one stable shareable URL no matter how often it re-renders, while changed content under one title lands beside the old file instead of clobbering it (an old link keeps working; the new render gets a fresh one). The **download** control still saves under the plain slug (`dash-board.html`), since it writes client-side and needs no uniqueness. One config switch gates the whole feature: `exports: false` stops the fanout, unregisters the route, and — through an announcement the host pushes onto the served page's boot table — hides the share control, so a disabled deployment never offers a dead URL. The feature also mounts only where a web server exists, so an export exists exactly when it is shareable — a TUI or headless profile keeps the untouched, filesystem-free tool. Because digest names accumulate rather than overwrite, retention replaces self-cleaning: `maxExportAgeDays` (default `7`, `0` disables) sweeps expired exports once at fanout activation.
