...
**Artifact gallery.** An **Artifacts** tab sits beside Chat (and the trajectory view, when your harness build has one): every finalized export the host currently holds, newest first, each with **Open** and **Copy link**. It reads the same exports directory the Share control opens into, so anything ever rendered and shared in the session stays reachable even after its original chat message scrolls out of view. Nothing pushes updates into an open tab — a manual **Refresh** picks up renders that settled after it was last loaded. The tab itself only appears where `shareArtifacts` is enabled, same as the card's Share control.
