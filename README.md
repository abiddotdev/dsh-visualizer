...
A settled card offers five actions — **Fullscreen** (expands the card to the viewport; Escape or a second click leaves), **Download** (self-contained `.html`/`.svg` via Blob, no server round-trip), **Copy HTML**, **Copy share link**, and **Share** (next section). When a document fails — a library that never loaded, or a script that threw — the card's error notice grows a sixth: one click that asks the model to repair the render, since the settle-time check compiles scripts without running them and so never sees these failures itself.

## Sharing

Every successful render is mirrored into the artifacts folder (`artifactDir`, default `~/.dsh/visualizer/artifacts`, private perms) the moment it finishes — a failed or interrupted render leaves nothing behind. A settled card carries two controls over that copy: **Share** opens it in a new tab at `/artifacts/visualizer/<name>?k=<key>`, and **Copy share link** puts that same address on the clipboard without leaving the chat.
