...
模型可见的 `visualizer` 工具：内联 HTML 呈现的参数流式半边。模型把完整自包含文档作为 `html` 参数直接传入（schema 中排在最后），浏览器卡片（`@deepseek-ai/dsh-client-ui-generativeui`）在模型还在书写时就对不断增长的调用参数前缀做解码，绘出实时预览。全程不触碰工作区：文档的持久载体就是记录在案的 `tool/call` 参数本身。
