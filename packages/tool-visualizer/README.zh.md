# @deepseek-ai/dsh-tool-generativeui

[English](README.md) | 中文

模型可见的 `render_html` 工具：内联 HTML 呈现的参数流式半边。模型把完整自包含文档作为 `html` 参数直接传入（schema 中排在最后），浏览器卡片（`@deepseek-ai/dsh-client-ui-generativeui`）在模型还在书写时就对不断增长的调用参数前缀做解码，绘出实时预览。全程不触碰工作区：文档的持久载体就是记录在案的 `tool/call` 参数本身。

**文档即调用。** `execute` 只校验参数携带的内容——非空文档、可配置的 `maxHtmlBytes` 上限（默认 256 KiB，按 UTF-8 字节计）、50–2000 px 的整数帧高、非空白的显式标题——并返回一行规范结果（`Rendered <title> (<bytes> bytes, <height>px frame)`）。不注入文件系统服务，不投影呈现元数据：落定后的卡片读取的是同一份已记录参数。
...
### `render_html` 工具 schema

#### 模型看到什么

三个原始参数——`title`（可选字符串）、`height`（可选整数）、`html`（必填字符串，声明在最后）——以及生成的[工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tool-generativeui)里的描述。
...
- **无自动高度适配**——帧高是模型的静态建议；实时卡片按该高度绘制流式前缀，文档自适应高度的桥接与 `show_html` 一样延后。
