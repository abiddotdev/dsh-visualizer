...
Streaming inline HTML card, browser half: the live `visualizer-stream` chat node that previews a `visualizer` call while the model writes it, plus the keyed `visualizer` tool row that takes over when the call dispatches.
