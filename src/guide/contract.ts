...
export const CONTRACT: readonly string[] = [
  ...
  'Two token families carry different jobs: --dsw-alias-* (label-primary/secondary/tertiary, state-error/success/warn-*, brand-*, bg-*) re-resolve when the theme flips — use them for all UI chrome; --dsw-static-{ramp}-{stop} ramps (deepseek, blue, green, amber, red, neutral-bluish) never flip — use them for data encoding, where a bar or edge must keep its color across themes. Light-mode component: fill at a -100 stop, border -600, text -900; dark-mode: fill -900, text -100; data marks read any stop directly.',
  'On canvas, resolve tokens once at startup and cache: const styles = getComputedStyle(document.documentElement); const ink = styles.getPropertyValue("--dsw-alias-label-primary").trim() — the canvas context cannot resolve var() itself, and re-reading per draw call lays a style recalc into every frame.',
